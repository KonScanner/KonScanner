...
<p align="center">
  ...
  <a href="https://konscanner.github.io/CV/"><img alt="CV" src="https://img.shields.io/badge/CV-konscanner.github.io-1f6feb?style=flat-square&logo=readthedocs&logoColor=white"></a>
...
| :--- | :--- | :--- |
| 2025 to May 2026 | **[Share.xyz](https://about.share.xyz/)** | Software Engineer. Owned the actions service and stayed its main maintainer: the Python backend that turns raw chain activity into the feed the app renders, and the API the iOS client reads. When the product turned to **prediction markets** I built that side too: the Polymarket data model, the live trade crawler, and eighteen endpoints covering events, markets and series, sports schedules, odds charts, positions and bettors. Block crawlers on a locked PostgreSQL cursor, replay on a five column key, and a repair service for stuck blocks. |
| Mar 2023 to 2025 | **[ZettaBlock](https://zettablock.com/)** | Senior Data Engineer, Data Scientist and Analytics Engineer. Owned the real time half of the platform: twelve chains held in PostgreSQL at the tip, with balances and query routing on top. Raised the decode rate on traces and logs by keying decoding to the signature and not the contract, so a call from a contract with no published ABI still came back as columns. Real time **Ingestor** and **Balances** on PostgreSQL with `asyncio`, checked against `balanceOf` at the same block. Specified the complexity heuristic that dispatched each query between Presto, Athena and PostgreSQL, so nobody had to choose an engine. dbt for transforms, MySQL for user management. |
| Apr 2022 to Apr 2023 | **[Polygon Labs](https://polygon.technology/about)** | Data Lead, Data Scientist and Analytics Engineer. Owned the Network, DeFi, NFT and Gaming data: collected it, decoded it, automated the pipeline, and set the metric definitions. 140 dbt models and 25 Airflow DAGs replaced metrics computed by hand, so a figure in a deck and the same figure on a dashboard finally agreed. Wallet balances from signed deltas in BigQuery, so the models never call the chain. Moved the heavy Python jobs to PySpark on Dataproc. The same models fed Looker and the tables that went to business development, marketing and partners including **Starbucks** and **Reddit**. |
